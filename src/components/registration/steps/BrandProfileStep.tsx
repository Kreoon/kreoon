...
export function BrandProfileStep({ data, onChange, onNext, onBack, mode }: StepComponentProps) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground mb-5">Cuéntanos más sobre tu empresa para conectarte con el talento ideal</p>

      {/* Industry */}
      <div className="mb-5">
        <label className="block text-xs font-medium text-foreground/80 mb-2">Industria</label>
      ...
      <div className="mb-5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-sm border border-border bg-white/5 pl-10 pr-3 py-2.5 text-sm text-white placeholder:text-gray-500 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30"
          />
        </div>
      </div>
      ...
      <button
        onClick={handleNext}
        className="w-full rounded-sm bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2.5 text-sm transition-colors"
      >
        Continuar
      </button>
    </motion.div>
