...
/**
 * Banner XL Duolingo-style "Continúa aprendiendo".
 * Thumbnail grande a la izquierda + CTA emocional a la derecha.
 */
export function ContinueLearningBigCard({
  ...
  const motivation =
    pct >= 90
      ? `🏆 ¡Estás a un paso del trofeo!`
      : pct >= 50
        ? `🔥 Vas por el ${pct}%, sigue así`
        : pct >= 20
          ? `🚀 Llevas un buen ritmo`
          : `✨ Acabas de empezar, ¡vamos!`;
    ...
    >
      <div className="flex flex-col md:flex-row gap-0 md:gap-6">
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-transparent to-black/30 md:to-black/0 pointer-events-none" />
        ...
        <div className="flex-1 p-5 md:p-6 flex flex-col justify-center gap-3">
          <div className="text-[11px] font-bold uppercase tracking-widest" style={{ color: accentColor }}>
          ...
          {nextLessonTitle && (
            <p className="text-sm text-zinc-400">
              Próxima: <span className="text-zinc-200 font-medium">{nextLessonTitle}</span>
            </p>
          )}
