...
function StatsPanel({ content }: { content: Content[] }) {
  ...
  const stats = [
    { label: "Total", value: content.length, icon: Play, color: "hsl(270 100% 60%)" },
    { label: "Valor Total", value: `$${totalValue.toLocaleString()}`, icon: DollarSign, color: "hsl(160 100% 45%)" },
    { label: "Pagados", value: paidCount, icon: TrendingUp, color: "hsl(190 100% 50%)" },
    { label: "Promedio", value: `$${avgPayment.toFixed(0)}`, icon: Star, color: "hsl(40 100% 50%)" },
  ];
...
function DistributionChart({ content }: { content: Content[] }) {
  ...
  const statusColors: Record<string, string> = {
    approved: "hsl(160 100% 45%)",
    paid: "hsl(270 100% 60%)",
    delivered: "hsl(190 100% 50%)",
    recording: "hsl(40 100% 50%)",
    assigned: "hsl(200 100% 50%)",
    editing: "hsl(300 100% 50%)",
    issue: "hsl(350 100% 50%)",
  };
          ...
          >
            ...
            <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
              <motion.div
                ...
                style={{ 
                  backgroundColor: statusColors[status] || "hsl(270 100% 60%)",
                  boxShadow: `0 0 10px ${statusColors[status] || "hsl(270 100% 60%)"}66`,
                }}
                initial={{ width: 0 }}
            ...
            <span 
              ...
              style={{ color: statusColors[status] || "hsl(270 100% 60%)" }}
            >
              {count}
            </span>
          </motion.div>
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] bg-background border-[hsl(270,100%,60%,0.2)] overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <motion.div
            ...
            style={{ background: "radial-gradient(circle, hsl(270 100% 60% / 0.2), transparent 70%)" }}
            animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }}
            transition={{ duration: 4, repeat: Infinity }}
          />
          <motion.div
            className="absolute -bottom-20 -left-20 w-48 h-48 rounded-full blur-[80px]"
            style={{ background: "radial-gradient(circle, hsl(190 100% 50% / 0.15), transparent 70%)" }}
        ...
        <div 
          ...
          style={{
            backgroundImage: `linear-gradient(hsl(270 100% 60%) 1px, transparent 1px),
                             linear-gradient(90deg, hsl(270 100% 60%) 1px, transparent 1px)`,
            ...
            <motion.div
              ...
              animate={{
                boxShadow: [
                  ...
                  "0 0 20px 5px hsl(270 100% 60% / 0.3)",
                  "0 0 0 0 hsl(270 100% 60% / 0)",
                ],
              }}
