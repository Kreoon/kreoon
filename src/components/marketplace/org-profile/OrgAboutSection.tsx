    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {org.org_year_founded && (
          <div className="flex items-center gap-3 p-3 rounded-sm bg-white/5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {org.org_specialties.map(spec => (
              <span key={spec} className="px-3 py-1.5 rounded-sm text-sm capitalize bg-white/5 text-foreground/80 border border-white/5">
                {spec}
              </span>
            ))}
          </div>
