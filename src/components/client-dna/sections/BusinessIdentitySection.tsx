  ...
  return (
    <div className="space-y-6">
      ...
      {(data.origin_story || isEditing) && (
        <div className="relative p-4 rounded-xl bg-gradient-to-br from-purple-500/10 to-pink-500/10
                        border border-purple-500/20">
          <div className="absolute top-3 right-3">
            <Quote className="w-8 h-8 text-purple-500/30" />
          </div>
          <p className="text-xs text-purple-400 uppercase tracking-wider mb-2">Historia de Origen</p>
          {isEditing ? (
      ...
      {(data.mission || isEditing) && (
        <div className="p-4 rounded-xl bg-white/5 border border-white/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-gray-500 uppercase tracking-wider">Misión</p>
          </div>
          {isEditing ? (
      ...
      {(data.unique_factor || isEditing) && (
        <div className="p-4 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-amber-400 uppercase tracking-wider">Factor Único</p>
          </div>
          {isEditing ? (
            <EditableText value={data.unique_factor} onChange={change('unique_factor')} placeholder="Factor único..." />
          ) : (
            <p className="text-sm text-white font-medium">{data.unique_factor}</p>
          )}
        </div>
      )}
      ...
      {(data.competitive_landscape || isEditing) && (
        <div className="p-4 rounded-xl bg-white/5 border border-white/10">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Panorama Competitivo</p>
          {isEditing ? (
