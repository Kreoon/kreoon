      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {(price || isEditing) && (
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800">
            <p className="text-xs text-green-600 dark:text-green-400 uppercase tracking-wider mb-1">Precio</p>
            {isEditing ? (
              <EditableText value={data.price || data.price_range} onChange={change('price') as (v: string) => void} placeholder="$..." />
            ) : (
              <p className="text-lg font-bold text-green-600 dark:text-green-400">{price}</p>
            )}
          </div>
        )}
      </div>
      ...
      {(data.price_justification || isEditing) && (
        <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm font-medium text-amber-600 dark:text-amber-400">Justificación del Precio</p>
          </div>
          {isEditing ? (
          ...
          {(data.guarantees?.length > 0 || isEditing) && (
            <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-medium text-blue-600 dark:text-blue-400">Garantías</p>
              </div>
              {isEditing ? (
          ...
          {(data.urgency_elements?.length > 0 || isEditing) && (
            <div className="p-4 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-medium text-red-600 dark:text-red-400">Elementos de Urgencia</p>
              </div>
              {isEditing ? (
