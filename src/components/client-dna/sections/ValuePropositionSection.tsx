  ...
  return (
    <div className="space-y-6">
          ...
          {(data.main_problem_solved || isEditing) && (
            <div className="p-4 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-medium text-red-600 dark:text-red-400">Problema que Resuelves</p>
              </div>
              {isEditing ? (
          ...
          {(data.solution_description || isEditing) && (
            <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-medium text-green-600 dark:text-green-400">Tu Solución</p>
              </div>
              {isEditing ? (
      ...
      {(data.transformation_promise || isEditing) && (
        <div className="relative p-6 rounded-lg bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 overflow-hidden">
          <div className="absolute top-2 right-2">
            <Sparkles className="w-6 h-6 text-purple-500/30" />
          </div>
          <p className="text-xs text-purple-600 dark:text-purple-400 uppercase tracking-wider mb-2">Promesa de Transformación</p>
          {isEditing ? (
          ...
          {isEditing ? (
            ...
            <div className="flex flex-wrap gap-2">
              {data.differentiators?.map((d, i) => (
                <span key={i} className="px-3 py-1 rounded-full bg-pink-100 dark:bg-pink-500/10 border border-pink-200 dark:border-pink-500/20 text-xs text-pink-600 dark:text-pink-300">
                  {d}
                </span>
              ))}
            </div>
