-- ============================================================
-- ADD: portfolio_thumbnail como fallback para avatar
-- Migration: 20260305790000_add_portfolio_thumbnail_to_search
-- ============================================================

-- DROP primero porque cambiamos el tipo de retorno
...
RETURNS TABLE (
  ...
  portfolio_thumbnail text,  -- NUEVO: primer thumbnail del portafolio
...
BEGIN
  ...
  WITH ranked_creators AS (
    SELECT DISTINCT ON (cp.id)
      ...
      cp.is_verified,
      cp.portfolio_count,
      -- Buscar primer thumbnail: portfolio_items > content > portfolio_posts
      COALESCE(
        (SELECT pi.thumbnail_url FROM portfolio_items pi
         WHERE pi.creator_id = cp.id AND pi.is_public = true AND pi.thumbnail_url IS NOT NULL
         ORDER BY pi.is_featured DESC, pi.display_order ASC LIMIT 1),
        (SELECT c.thumbnail_url FROM content c
         WHERE c.creator_id = cp.user_id AND c.is_published = true AND c.thumbnail_url IS NOT NULL
         ORDER BY c.created_at DESC LIMIT 1),
        (SELECT pp.thumbnail_url FROM portfolio_posts pp
         WHERE pp.user_id = cp.user_id AND pp.thumbnail_url IS NOT NULL
         ORDER BY pp.created_at DESC LIMIT 1),
        -- Si no hay thumbnail, usar la URL del media directamente (para imágenes)
        (SELECT pi.media_url FROM portfolio_items pi
         WHERE pi.creator_id = cp.id AND pi.is_public = true AND pi.media_type = 'image'
         ORDER BY pi.is_featured DESC, pi.display_order ASC LIMIT 1),
        (SELECT pp.media_url FROM portfolio_posts pp
         WHERE pp.user_id = cp.user_id AND pp.media_type = 'image'
         ORDER BY pp.created_at DESC LIMIT 1)
