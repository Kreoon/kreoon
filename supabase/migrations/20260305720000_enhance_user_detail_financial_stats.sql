-- =====================================================
-- Mejorar get_full_user_detail con estadísticas financieras calculadas
-- Migration: 20260305720000_enhance_user_detail_financial_stats
-- =====================================================
...
BEGIN
  ...
  -- Calculate total earned from wallet transactions (credits)
  SELECT COALESCE(SUM(amount), 0) INTO v_total_earned
  FROM wallet_transactions
  WHERE user_id = p_user_id
    AND type IN ('credit', 'deposit', 'payment_received', 'earning');

  -- Calculate total spent from wallet transactions (debits)
  SELECT COALESCE(SUM(amount), 0) INTO v_total_spent
  FROM wallet_transactions
  WHERE user_id = p_user_id
    AND type IN ('debit', 'withdrawal', 'payment_sent', 'purchase');
  ...
  SELECT jsonb_build_object(
    ...
    -- Calculated values (override table values)
