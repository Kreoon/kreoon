-- =====================================================
-- Fix: Recrear get_full_user_detail con verificación de columnas
-- Migration: 20260305200000_fix_get_full_user_detail_v2
-- =====================================================

-- Primero, asegurarse de que las columnas existen
DO $$
BEGIN
  -- Verificar y agregar nationality si no existe
  ...
  -- Verificar y agregar onboarding_completed si no existe
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema = 'public'
                 AND table_name = 'profiles'
...
DECLARE
  ...
  arrays_data JSONB;
BEGIN
  -- Part 1: Basic profile fields (including nationality, date_of_birth, onboarding_completed)
  ...
  -- Part 3: Profile extras (scores, flags, is_banned, is_platform_admin)
  SELECT jsonb_build_object(
