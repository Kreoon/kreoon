-- ============================================================================
-- Talent Marketplace Pause System
-- Adds marketplace_paused_until to creator_profiles and updates get_unified_talent
-- ============================================================================

ALTER TABLE public.creator_profiles
  ADD COLUMN IF NOT EXISTS marketplace_paused_until timestamptz NULL;

COMMENT ON COLUMN public.creator_profiles.marketplace_paused_until IS
  'NULL = visible; future date = temporalmente pausado; NULL + is_active=false = desactivado permanentemente';

-- ============================================================================
-- Update get_unified_talent to include marketplace pause fields
-- ============================================================================
...
RETURNS TABLE (
  ...
  marketplace_paused_until timestamptz
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    p.community_name,
    p.referred_by,
    -- Marketplace availability: active if is_active=true AND no active pause
    COALESCE(
