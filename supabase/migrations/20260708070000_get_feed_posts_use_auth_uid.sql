-- SEGURIDAD (hallazgo de revision post-commit): get_feed_posts recibia p_viewer_id desde el
-- cliente. Al ser SECURITY DEFINER con grant a authenticated, cualquier usuario autenticado
-- podia pasar el uuid de OTRO usuario y leer su is_saved/is_liked/reaction, y colaba su propia
-- org/followers en las reglas de visibilidad (IDOR). Fix: se elimina el parametro p_viewer_id
-- por completo; el viewer se resuelve exclusivamente con auth.uid() dentro de la funcion.

drop function if exists public.get_feed_posts(uuid, text, text, timestamptz, uuid, integer);
...
begin
  ...
  select
    ...
    (case when v_viewer_id is null then false
     when u.post_source = 'portfolio_item' then
      exists (select 1 from public.feed_reactions fr where fr.post_id = u.post_id and fr.user_id = v_viewer_id)
    else
      exists (select 1 from public.portfolio_post_likes ppl where ppl.post_id = u.post_id and ppl.viewer_id = v_viewer_id::text)
    end) as is_liked,
    (case when v_viewer_id is null then null when u.post_source = 'portfolio_item' then
      (select fr.reaction_type from public.feed_reactions fr where fr.post_id = u.post_id and fr.user_id = v_viewer_id limit 1)
    else null end) as my_reaction,
    (v_viewer_id is not null and exists (
      select 1 from public.saved_items si
      where si.user_id = v_viewer_id and si.item_id = u.post_id
...
grant execute on function public.get_feed_posts(text, text, timestamptz, uuid, integer) to authenticated;
