-- Incluir fillmaker_services en get_org_finance_overview
-- Fix A: el price_client de Fillmakers suma como ingreso de la org,
--        el cost_own suma como nómina (solo en COP).

DROP FUNCTION IF EXISTS public.get_org_finance_overview(UUID, DATE, DATE, TEXT);

CREATE FUNCTION public.get_org_finance_overview(
  p_org_id    UUID,
  p_start     DATE,
  p_end       DATE,
...
DECLARE
  ...
  -- Fillmaker
  v_fill_sold         NUMERIC := 0;
  v_fill_collected    NUMERIC := 0;
  v_fill_pending      NUMERIC := 0;
  v_fill_payroll      NUMERIC := 0;
...
BEGIN
  ...
  -- ── Fillmaker: vendido, cobrado, pendiente ───────────────────────────────────
  SELECT COALESCE(SUM(price_client), 0), COUNT(*)
  INTO v_fill_sold, v_fill_count
  ...
  v_sold      := v_sold      + v_fill_sold;
  v_collected := v_collected + v_fill_collected;
  v_pending   := v_pending   + v_fill_pending;
  ...
  IF p_currency = v_talent_cur THEN
    ...
    -- Fillmaker: cost_own pagado en período (solo COP)
    SELECT COALESCE(SUM(cost_own), 0)
    INTO v_fill_payroll
    FROM fillmaker_services
    WHERE organization_id = p_org_id
      AND COALESCE(currency, 'COP') = v_talent_cur
    ...
    v_payroll := v_payroll_tp + v_payroll_content + v_fill_payroll;
  END IF;
